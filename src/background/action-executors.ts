import type { ActionExecutionResponse, ExecuteActionMessage } from "@/shared/extension-messages";
import { normalizeHttpUrl } from "@/shared/url-action";
import type { AgentAction, AgentActionResult, AgentToolName } from "@/shared/types";

type BrowserActionExecutor = (action: AgentAction) => Promise<ActionExecutionResponse>;
type PageActionDispatcher = (message: ExecuteActionMessage) => Promise<ActionExecutionResponse>;
...
async function executeOpenUrlAction(action: AgentAction): Promise<ActionExecutionResponse> {
...
const BROWSER_ACTION_EXECUTORS: Partial<Record<AgentToolName, BrowserActionExecutor>> = {
...
export async function executeAgentAction(
  ...
  if (action.riskLevel === "high") {
    return {
      ok: true,
      data: createActionResult(action, "blocked", "高风险动作已被本地策略阻断。")
    };
  }

  const browserExecutor = BROWSER_ACTION_EXECUTORS[action.toolName];
  return browserExecutor ? browserExecutor(action) : dispatchPageAction(message);
}
